...
class RuleReader private constructor(
    ...
    fun getRule(libName: String, @LibType type: Int, useRegex: Boolean,
                remoteRepo: LCRemoteRepo = LCRemoteRepo.GitHub, language: String = "en"): Rule? {
        ...
        return record?.let {
            val label = if (language.substringBefore('-').substringBefore('_').equals("zh", ignoreCase = true)) it.label else it.labelEn ?: it.label
            Rule(libName, it.type, label, IconResMap.getIconRes(it.iconIndex),
    ...
    companion object {
        ...
        fun open(databaseFile: File): RuleReader {
            ...
            try {
                ...
                require(columns == requiredColumns || columns == requiredColumns + "labelEn") { "Expected compact v5 rule columns" }
        ...
        private fun Cursor.toRecord(): RuleRecord {
            ...
            return RuleRecord(requiredString("name"), requiredString("label"), type.toInt(), iconIndex.toInt(),
                ...
                if (getColumnIndex("labelEn") >= 0) nullableString("labelEn") else null)
        }
