...
class RuleReaderTest {
    ...
    @Test fun selectsLabelPerCallAndFallsBackWithoutEnglish() {
        val db = fixture()
        insert(db, 1, "localized", false, 1)
        insert(db, 2, "fallback.*", true, 2)
        metadata(2)
        update(db, "ALTER TABLE rules_table ADD COLUMN labelEn TEXT")
        update(db, "UPDATE rules_table SET label='中文', labelEn='English' WHERE _id=1")
        LCRules.activateDatabase(db)
        assertEquals("English", query("localized")!!.label)
        for (language in listOf("zh", "zh-Hans", "zh-Hant", "ZH-cn", "zh_Hans", "en", "fr", "", "zh-Hans", "en")) {
            assertEquals(if (language.substringBefore('-').substringBefore('_').equals("zh", ignoreCase = true)) "中文" else "English", query("localized", language)!!.label)
            assertEquals("2", query("fallback-name", language)!!.label)
        }
        RuleReader.open(db).use { reader ->
            assertEquals("English", reader.getRule("localized", NATIVE, false)!!.label)
            assertEquals("中文", reader.getRule("localized", NATIVE, false, language = "zh")!!.label)
        }
        update(db, "UPDATE rules_table SET labelEn=x'01' WHERE _id=1")
    ...
    @Test fun readsRealDatabaseOnlyArtifact() {
        ...
        RuleReader.open(File(root, "rules.db")).use { reader ->
            ...
            SQLiteDatabase.openDatabase(File(root, "rules.db").path, null, SQLiteDatabase.OPEN_READONLY).use { source ->
                source.rawQuery("SELECT name,type,label,labelEn FROM rules_table", null).use { rows ->
                    while (rows.moveToNext()) {
                        assertEquals(rows.getString(2), reader.getRule(rows.getString(0), rows.getInt(1), false, language = "zh")?.label)
                        assertEquals(rows.getString(3) ?: rows.getString(2), reader.getRule(rows.getString(0), rows.getInt(1), false)?.label)
                    }
                }
            }
        }
    ...
    private fun query(name: String, language: String = "en"): Rule? {
        var outcome: Result<Rule?>? = null
        suspend { LCRules.getRule(name, NATIVE, true, language) }.startCoroutine(object : Continuation<Rule?> {
